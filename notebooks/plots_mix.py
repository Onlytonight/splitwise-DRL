                ...
                for req_type in req_types:
                    ...
                    if not type_filtered_df.empty:
                        for quantile in quantiles:
                            result[f"{req_type}_ttft_slowdown_p{int(quantile * 100)}"] = type_filtered_df[
                                "ttft_slowdown"].quantile(quantile)
                            result[f"{req_type}_tbt_slowdown_p{int(quantile * 100)}"] = type_filtered_df[
                                "tbt_slowdown"].quantile(quantile)
                            result[f"{req_type}_e2e_slowdown_p{int(quantile * 100)}"] = type_filtered_df[
                                "e2e_slowdown"].quantile(quantile)

                        # 对于时间指标，我们使用原始值而不是slowdown
                        for quantile in quantiles:
                            result[f"{req_type}_ttft_times_p{int(quantile * 100)}"] = type_filtered_df[
                                "ttft_times"].quantile(quantile)
                            result[f"{req_type}_tbt_times_p{int(quantile * 100)}"] = type_filtered_df[
                                "tbt_times"].quantile(quantile)
                            result[f"{req_type}_e2e_times_p{int(quantile * 100)}"] = type_filtered_df[
                                "response_times"].quantile(quantile)
                            result[f"{req_type}_nth_token_overheads_p{int(quantile * 100)}"] = type_filtered_df[
                                "nth_token_overheads"].quantile(quantile)
                            result[f"{req_type}_queue_times_p{int(quantile * 100)}"] = type_filtered_df[
                                "queue_times"].quantile(quantile)

                # 添加所有请求类型的总体指标作为第三条曲线
                for quantile in quantiles:
                    result[f"all_ttft_slowdown_p{int(quantile * 100)}"] = request_df["ttft_slowdown"].quantile(quantile)
                    result[f"all_tbt_slowdown_p{int(quantile * 100)}"] = request_df["tbt_slowdown"].quantile(quantile)
                    result[f"all_e2e_slowdown_p{int(quantile * 100)}"] = request_df["e2e_slowdown"].quantile(quantile)

                    # 对于时间指标，我们使用原始值而不是slowdown
                    result[f"all_ttft_times_p{int(quantile * 100)}"] = request_df["ttft_times"].quantile(quantile)
                    result[f"all_tbt_times_p{int(quantile * 100)}"] = request_df["tbt_times"].quantile(quantile)
                    result[f"all_e2e_times_p{int(quantile * 100)}"] = request_df["response_times"].quantile(quantile)
                    result[f"all_nth_token_overheads_p{int(quantile * 100)}"] = request_df[
                        "nth_token_overheads"].quantile(quantile)
                    result[f"all_queue_times_p{int(quantile * 100)}"] = request_df["queue_times"].quantile(quantile)
...
def plot_y_vs_trace_by_request_type(results_df,
    ...
    colors = ['blue', 'orange', 'green']  # 添加绿色用于总体指标
    markers = ['o', 's', '^']  # 添加三角形标记用于总体指标
    ...
    for y_var in y_vars:
        for quantile in quantiles:
            y_col = f"all_{y_var}_p{int(quantile * 100)}"
            if y_col in results_df.columns:
                sns.lineplot(data=results_df,
                             x="trace",
                             y=y_col,
                             color=colors[2],  # 绿色
                             marker=markers[2],  # 三角形标记
                             markersize=7,
                             label="all",
                             ax=axs[y_vars.index(y_var)][quantiles.index(quantile)])
            else:
